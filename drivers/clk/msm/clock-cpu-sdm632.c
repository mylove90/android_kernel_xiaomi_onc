...
ssize_t get_Voltages(char *buf)
{
	...
	if (!buf)
		return 0;
	//Ceiling
	for (i = 1; i < pwr_clk.c.num_fmax; i++) {
		uv = cpr_regulator_get_ceiling_voltage(
					pwr_clk.c.vdd_class->regulator[0],
					pwr_clk.c.vdd_class->vdd_uv[i]);
		if (uv < 0) return 0;
		count += sprintf(buf + count, "pwrcl_Vmax:%lumhz: %d mV\n",
					pwr_clk.c.fmax[i] / 1000000,
					uv / 1000);
	//Floor
	...
	//Big ceiling
	for (i = 1; i < perf_clk.c.num_fmax; i++) {
		uv = cpr_regulator_get_ceiling_voltage(
					perf_clk.c.vdd_class->regulator[0],
					perf_clk.c.vdd_class->vdd_uv[i]);
		if (uv < 0)
			return 0;
		count += sprintf(buf + count, "perfcl_Vmax:%lumhz: %d mV\n",
					perf_clk.c.fmax[i] / 1000000,
					uv / 1000);
	//floor
		uv = cpr_regulator_get_floor_voltage(
					perf_clk.c.vdd_class->regulator[0],
